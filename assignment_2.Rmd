...
continent_TabData <- Continent_data %>%
  ...
  group_by(Entity) %>% 
  summarise(
    "Population 1950" = comma(Population[Year == 1950]),
...
ggplot(Population_DevelopmentData, aes(x = Year, y = Population, color = Socio_Economic_Status, fill = Socio_Economic_Status)) +
  geom_area() +
  facet_wrap(~ AgeGroup)
```
