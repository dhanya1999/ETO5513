...
### How does the gender gap in STEM education compare to other fields globally between 2015 and 2022?
...
### How does gender parity in STEM education compare to non-STEM fields in Australia across different levels and fields from 2015 to 2022?
...
#
# Vector of the required columns.
#
...
# 
# Define a vector of STEM-related keywords to search for to classify field as stem or not.
#
 stem_keywords <- c("Science", "Technology", "Engineering", "Mathematics", "ICT", "Veterinary")

#
# Add a new column 'STEM_Status' categorizing as 'STEM' or 'OTHER'.
...
#
# Get the unique field areas being compared as part of Gender Gap.
#
unique_fields <- unique(rn2$Fld)

#
# Convert the unique fields into a data frame for display as a formatted table.
#
unique_fields_df <- data.frame(FieldArea = unique_fields)

#
# Rename the column from 'FieldArea' to 'Field Area'.
#
colnames(unique_fields_df)[colnames(unique_fields_df) == "FieldArea"] <- "Field Area"

#
# Calculate the summary statistics for gender gap by grouping by STEM_Status.
...
#
# Calculate Q1 (25%) and Q3 (75%) for Gender_Gap grouped by STEM_Status.
#
iqr_stats <- rn2 %>%
  group_by(STEM_Status) %>%
  summarise(
...
#
# Australia only rows for visualising the Australia data:
...
```{r Figure-Gngp, warning=FALSE, fig.cap="World STEM Gender Gap(%)<br>(Based on 2015-2022 Data)", fig.topcaption=TRUE}
...
```{r Figure-Astrl,warning=FALSE,fig.cap="Gender Gap in STEM Education <br>by<br> Field and Level in Australia (2015–2022)",fig.topcaption=TRUE}
...
```{r  table-FldCmpr, warning=FALSE, tab.cap="Fields Compared for Gender Gap(%) Analysis<br>(Based on 2015-2022 Data).", tab.cap.style = "inline"}
...
```{r table-SmmrySttstcs, warning=FALSE, tab.cap="World STEM Gender Gap(%) Statistics<br>(Based on 2015-2022 Data).", tab.cap.style = "inline"}
...
```{r table-Astrltb, warning=FALSE, tab.cap="Gender Gap in STEM Education <br>by<br> Field and Level in Australia (2015–2022) that Approximates Parity", tab.cap.style = "inline"}
...
with $∥GenderGap∥\space ≤\space 1.0$ indicating near-equal representation. For example, in the Agriculture, Forestry, Fisheries, and Veterinary category, all levels except PhD demonstrate approximate parity.  In contrast, females dominate the PhD level (Table \@ref(tab:table-Astrltb)). The table also highlights other fields and education levels where the gender gap is near parity.

Non-STEM (OTHER) fields demonstrate a contrasting trend, with a pronounced negative gender gap indicating female dominance. For instance, health and education fields exhibit significantly higher female representation across all levels, particularly in Bachelor’s and Master’s degrees (Figure \@ref(fig:Figure-Astrl)). Variability in non-STEM fields reflects broader gender diversity, with some areas achieving parity or female dominance while others differ significantly from STEM. OTHERS differ significantly from STEM, where male dominance is more uniform and persistent, requiring targeted efforts to address gender disparities. The broader range of gender gap values in non-STEM fields underscores this diversity compared to STEM. These findings emphasize the need for targeted initiatives to encourage female participation in STEM while maintaining equity in non-STEM areas.
